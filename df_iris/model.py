...
class MultiBranchResNet(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.local_conv = nn.Sequential(
            nn.Conv2d(local_in_channels, 256, kernel_size=1, bias=False),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
        )
        self.local_fc = nn.Linear(256, proj_dim, bias=False)
        self.local_norm = nn.LayerNorm(proj_dim)

        self.token_learner = TokenLearner(256, num_tokens=num_tokens, hidden_dim=64, dropout_p=dropout_p)
        self.token_fusion_norm = nn.LayerNorm(num_tokens * 256)
        self.integrated_fc = nn.Linear(proj_dim + num_tokens * 256, proj_dim, bias=False)
    ...
    def _cam_cdm_mask(
        ...
        *,
        ...
        weight = F.normalize(self.head_global.weight, dim=1)
        ...
        raw_cdm = F.relu(target - negative_ref)
